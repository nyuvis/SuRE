...
function render_tree_path(leaf_id) {
	d3.selectAll('#tree_path > *:not(.depth-line)').remove();
	...
	let treeData = construct_tree_data(leaf_id);
	root1 = d3.hierarchy(treeData, function(d) { return d.children; });
	root1.x0 = tree_height / 4 * 3;
	root1.y0 = 0;

	// Compute the new tree layout.
	var treemap = d3.tree().size([graph_width, tree_height])
	tree_hierarchy = treemap(root1);

	// update_tree();
}

function construct_tree_data(leaf_id) {	
	let node = node_info[leaf_id]['parent'];
	let info = node_info[leaf_id];
	while (node > 0) {
		let new_info = Object.assign({}, node_info[node]);
		new_info["children"] = [info];
		info = Object.assign({}, new_info);
...
function update_tree() {
	...
	tree_nodes.selectAll('rect')
		.data(node => {
			let size = summary_size_(d3.sum(node['data']['value']));
    ...
    linktext.append("text")
		...
		.text(function (d) {
			...
			// }
			real_threshold = real_percentile['percentile_table'][Math.floor(cate_threshold)][parent['feature']];
			if (!Number.isInteger(real_threshold)) {
				real_threshold = real_threshold.toFixed(1)
			}
			return attrs[parent.feature]+d.data.sign + real_threshold;
		})
	...
	Object.keys(multiple_selection).forEach(node_id => {
		...
		let size = summary_size_(d3.sum(node_info[node_id]['value']))
		node.append('rect')
	...
	node_mask.append('rect')
		.attr('class', 'node_mask')
		.attr('x', d => -summary_size_(d3.sum(d['data']['value']))/2)
		.attr('y', d => -summary_size_(d3.sum(d['data']['value']))/2)
		.attr('width', d => summary_size_(d3.sum(d['data']['value'])))
		.attr('height', d => summary_size_(d3.sum(d['data']['value'])))
		.attr('visibility', (d) => {
	...
	node_mask.on('mouseover', function(node) {
		let d = node['data'];
